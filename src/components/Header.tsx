...
const Header = ({GOOGLE_DIRECTION_URL}: { GOOGLE_DIRECTION_URL: string }) => {
    ...
    const Nav = () => {
        return (<ul
            class={"flex bg-gray-dark sm:bg-transparent bg-opacity-80 sm:bg-opacity-100 absolute sm:relative pb-2 pl-2 top-12 sm:top-0 right-0 w-min lg:text-xl flex-col space-y-2 self-center text-right sm:flex-row md:w-full"}
        >
            <li class={"hidden"}/>
            <li
                id="home"
                class={`nav_link sm:w-auto ${route === "/" ? "active" : "not-active"}`}
            >
            ...
            <li
                id="packages"
                class={`nav_link h-full w-full sm:w-auto ${route.startsWith("/packages") ? "active" : "not-active"}`}
            >
                <a href="/packages">
                    Packages
                </a>
            ...
            <li
                id="faq"
                class={`nav_link h-full w-full sm:w-auto ${route.startsWith("/faq") ? "active" : "not-active"}`}
            >
    ...
    return (<div class="sticky flex mb-auto flex-row">
  ...
  width: 1px;*/}
            ...
            <div
                class={"hidden content-center items-end text-center sm:flex sm:w-full relative"}
            >
                <Nav/>
            </div>
            ...
            >
                <button
                    className={"flex 2xl:hidden w-full flex-row items-end justify-end text-right sm:hidden"}
